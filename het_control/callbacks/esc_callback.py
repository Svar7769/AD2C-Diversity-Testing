...
class ESCCallback(Callback):
    ...
    def on_evaluation_end(self, rollouts: List[TensorDictBase]) -> None:
        ...
        # Collect episode returns (sum over time, mean over agents to match "mean return")
        episode_rewards = []
        with torch.no_grad():
            for rollout in rollouts:
                reward_key = ('next', self.control_group, 'reward')
                if reward_key in rollout.keys(include_nested=True):
                    # Shape: [time_steps, n_agents, 1] or similar
                    rewards = rollout.get(reward_key)
                    # Sum over time (dim 0), mean over agents to get episode return
                    episode_return = rewards.sum(dim=0).mean().item()
        ...
        # Compute mean episode return across episodes (should match "mean return")
        ...
        print(
            ...
            f"Grad: {lpf_output:+.5f})  |"
            f"derivative: {demodulated:+.5f}"
        )
