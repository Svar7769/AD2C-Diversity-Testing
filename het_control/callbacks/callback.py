...
def get_het_model(policy):
    """
    Extract the heterogeneous control model from a policy.
    
    Handles both empirical and hierarchical models, and different policy structures.
    
    Args:
        policy: The policy object containing the model
        
    Returns:
        The HetControlMlp model (either Empirical or Hierarchical)
    """
    import torch.nn as nn
    
    model = policy.module
    
    # Unwrap until we find the actual model
    max_depth = 10  # Prevent infinite loops
    depth = 0
    
    while depth < max_depth and not isinstance(model, (HetControlMlpEmpirical, HetControlMlpHierarchical)):
        # Handle ModuleList, list, or tuple
        if isinstance(model, (nn.ModuleList, list, tuple)):
            model = model[0]
        # Handle objects with a 'module' attribute
        elif hasattr(model, 'module'):
            model = model.module
        # Handle TensorDictModule-like structures that have modules inside
        elif hasattr(model, '_modules') and len(model._modules) > 0:
            # Get the first module from the internal _modules dict
            first_key = list(model._modules.keys())[0]
            model = model._modules[first_key]
        else:
    ...
    if not isinstance(model, (HetControlMlpEmpirical, HetControlMlpHierarchical)):
        raise TypeError(
            f"Expected HetControlMlpEmpirical or HetControlMlpHierarchical, "
            f"but got {type(model).__name__}. "
            f"Policy structure: {type(policy.module).__name__}"
        )
...
class NormLoggerCallback(Callback):
    ...
    Updated to be robust against missing keys and hierarchical models.
    """

    def on_batch_collected(self, batch: TensorDictBase):
        """
        Log metrics from collected batch.
        
        IMPORTANT: This is called AFTER collection, not during training.
        We must be careful not to cause batch size mismatches.
        """
        for group in self.experiment.group_map.keys():
            # Define keys that should exist in all models
            standard_keys = [
                (group, "observation"),
            ]
            
            # Additional keys that might not always exist
            optional_keys = [
                (group, "f"),
                (group, "g"),
                (group, "fdivg"),
            ...
            for key in standard_keys:
                try:
                    ...
                    # Key doesn't exist or can't be accessed, skip it
                    pass
            
            # Try optional keys without failing
...
class ActionSpaceLoss(Callback):
    ...
    def on_train_step(self, batch: TensorDictBase, group: str) -> TensorDictBase:
        """
        IMPORTANT: This returns a NEW TensorDict, not modifying the input batch.
        """
        if not self.use_action_loss:
            return TensorDict({}, batch_size=batch.batch_size, device=batch.device)
        ...
        # Create a NEW TensorDict with ONLY the loss values
        # Do NOT modify the input batch!
        loss_td = TensorDict(
            {"loss_action_space": loss.detach()},  # Detach to prevent grad issues
