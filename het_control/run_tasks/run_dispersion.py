"""
Runner script for Dispersion task with ESC control.
...
TASK_OVERRIDES = {
    "n_agents": 3,
    # Add any specific dispersion parameters here if needed
}
...
ESC_OVERRIDES = {
    ...
    "integrator_gain": -10.0,     # Base gain for gradient descent
...
if __name__ == "__main__":
    ...
    if USE_ESC and ESC_OVERRIDES:
        try:
            ...
            temp_esc_config = "/tmp/escontroller_dispersion.yaml"
    ...
    print(f"{'='*80}")
    print(f"🎯 Running Dispersion Task with ESC")
    print(f"{'='*80}")
    ...
    print(f"   Dither: ±{ESC_OVERRIDES['dither_magnitude']} @ {ESC_OVERRIDES['dither_frequency']} rad/s")
    print(f"   SND bounds: [{ESC_OVERRIDES['min_snd']}, {ESC_OVERRIDES['max_snd']}]")
    print(f"{'='*80}\n")
