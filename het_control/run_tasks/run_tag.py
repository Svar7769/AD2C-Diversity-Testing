...
def parse_all_overrides():
    ...
    - task.*: Task parameters (e.g., num_adversaries, num_good_agents)
...
def run_tag_experiment(
    ...
    """
    Run Simple Tag experiment with specified parameters.
    ...
    if use_esc:
        print(f"\n🎛️  ESC Controller:")
        print(f"   Control group: {esc_overrides.get('control_group', 'adversary')}")
        ...
        print(f"   Integrator gain: {esc_overrides.get('integrator_gain', -1.0)}")
        ...
        print(f"   Policy freeze: {'Yes' if esc_overrides.get('simple_tag_freeze_policy', False) else 'No'} (after {esc_overrides.get('simple_tag_freeze_policy_after_frames', 1_000_000):,} frames)")
